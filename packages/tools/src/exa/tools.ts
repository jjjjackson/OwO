...
import type { ExaToolConfig } from "../types"
...
const webSearchDescription = `Search the web using Exa AI - performs real-time web searches and can scrape content from specific URLs.
...
const codeContextDescription = `Search and get relevant context for any programming task. Exa-code has the highest quality and freshest context for libraries, SDKs, and APIs.

**Use this tool for ANY question or task related to programming.**

**When to use:**
- When implementing unfamiliar APIs or libraries
- When Context7 is lacking examples or doesn't contain the library
- When you need the freshest documentation (recently updated libraries)
- When looking for real-world usage patterns
...
export function createExaTools(config?: ExaToolConfig): ExaTools {
  ...
  const exaSearch = tool({
    ...
    args: {
      ...
      livecrawl: tool.schema
        .enum(["fallback", "preferred"])
        .default("fallback")
        .describe(
          "Live crawl mode - 'fallback': use cached then live (default), 'preferred': prioritize live crawling",
        ),
    },
  ...
  const exaCodeContext = tool({
    ...
    args: {
      ...
      maxCharacters: tool.schema
        ...
        .default(10000)
        .describe(
          "Maximum characters of context to return (1000-50000, default: 10000). Use higher values for comprehensive documentation.",
        ),
    },
    async execute(args, ctx) {
      try {
